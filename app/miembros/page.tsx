        ...
        {step === 0 && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="flex flex-col items-center mb-8">
              ...
              <input id="foto-input-miembro" type="file" accept="image/*" className="hidden" onChange={handleFotoChange} />
              <input id="foto-input-miembro-camera" type="file" accept="image/*" capture="environment" className="hidden" onChange={handleFotoChange} />
              {fotoPreview ? (
                <button type="button" onClick={() => document.getElementById('foto-input-miembro')?.click()}
                  className="text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors">
                  Cambiar foto
                </button>
              ) : (
                <div className="flex items-center gap-3 mt-1">
                  <button type="button" onClick={() => document.getElementById('foto-input-miembro-camera')?.click()}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    Tomar foto
                  </button>
                  <button type="button" onClick={() => document.getElementById('foto-input-miembro')?.click()}
                    className="text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors underline underline-offset-2">
                    Elegir archivo *
                  </button>
                </div>
              )}
              {!fotoPreview && <p className="text-xs text-slate-400 mt-1">JPG, PNG, WEBP o HEIC · Máx 5MB</p>}
            </div>
